      ...
      {#each inputValue as task}
        {#if task}
          <div style="margin-bottom:2px;" class="list-item">
            ...
            {/if}
            <label class="label-task ml-2 text-xl list-card-details"  for="task"  on:click={showTasks}>{task.taskName} </label>
            <input class="task-modified border-gray-300 bg-gray-100 rounded-[10PX] w-1/6 px-1 py-1 mt-2 text-sm js-card-details" type="text">
            <button class="buttonDone bg-[#c4bcbc] text-black px-1 py-1 rounded-md text-sm js-card-details" on:click={saveTask(event, user)}>Done</button>
            {#if task.date}
              <i class="mi mi-calendar"><span class="u-sr-only" on:click={(event) => updateCalendar(event)}>{formatDate(task.date)}</span></i>
              <button  class="buttonDoneDate bg-[#c4bcbc] text-black px-1 py-1 rounded-md text-sm js-card-details" name="save" type="button" on:click={(event) => saveCalendar(event, user, date, dates, prevDate, prevDates)}>Save</button>
            {/if}
            {#if task.dates}
              ...
              <button  class="buttonDoneDates bg-[#c4bcbc] text-black px-1 py-1 rounded-md text-sm js-card-details" name="save" type="button" on:click={(event) => saveCalendar(event, user, date, dates, prevDate, prevDates)}>Save</button>
            {/if}
          </div>
        {/if}
      {/each}
