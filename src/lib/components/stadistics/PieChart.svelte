<script lang="ts">
  ...
  const width = 480 // outer width of the chart, in pixels
  ...
  const strokeWidth = 7; // width of stroke separating wedges
  ...
  const strokeColorWOR = ''; // stroke color when inner radius is greater than 0
  ...
  const x = Object.keys(dataPieChart[0])[0];
  const y = Object.keys(dataPieChart[0])[1];
  const xVals = dataPieChart.map((el) => el[x]);
  let yVals = dataPieChart.map((el) => Number(el[y]));
  if (percent) {
    const total = yVals.reduce((a, b) => a + b, 0);
    yVals = yVals.map((el) => el / total);
  }
  const iVals = dataPieChart.map((el, i) => i);
