<script>
  ...
  const width = 480 // the outer width of the chart, in pixels
  ...
  const innerRadius = 107; // the inner radius of the chart, in pixels
  ...
  const x = Object.keys(dataDonutChart[0])[0]; // given d in data, returns the (ordinal) x-value
  const y = Object.keys(dataDonutChart[0])[1]; // given d in data, returns the (quantitative) y-value
  const xVals = dataDonutChart.map((el) => el[x]);
  let yVals = dataDonutChart.map((el) => Number(el[y]));
  if (percent) {
    const total = yVals.reduce((a, b) => a + b, 0);
    yVals = yVals.map((el) => el / total);
  }
  const iVals = dataDonutChart.map((el, i) => i);
