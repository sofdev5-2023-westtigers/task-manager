<script>
  ...
  const yLabel = '↑ Time Work'; // label for the y-axis
  const color = '#ABEBC6'; // bar fill color
  const yScalefactor = 6; // number of ticks on y-yaxis

  // Sort data by default, ascending, or descending
  let sortedData = dataBarChart;
  $: reactiveShowSort = (input) => {
    if (input === 1) {
      return (sortedData = dataBarChart.sort(
        (a, b) => a[x].charCodeAt(0) - b[x].charCodeAt(0)
      ));
    }
    if (input === 2) {
      return (sortedData = dataBarChart.sort((a, b) => a[y] - b[y]));
    }
    if (input === 3) {
      return (sortedData = dataBarChart.sort((a, b) => b[y] - a[y]));
    }
  };

  // Compute values X and Y value of Arrays
  const x = Object.keys(dataBarChart[0])[0]; // given d in data, returns the (ordinal) x-value
  const y = Object.keys(dataBarChart[0])[1]; // given d in data, returns the (quantitative) y-value
