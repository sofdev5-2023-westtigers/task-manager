...
<li>
  ...
  >
    <svg
      ...
      /></svg
    >
    <span class="flex-1 ml-3 whitespace-nowrap">{lastName} {firstName}</span>
    <Button pill={true} outline={true} color="dark" class="!p-2" size="xs" on:click={() => {
      deleteMemberTaskList(listName, email, taskName);
      let memberList = getMembersTaskList(listName, taskName);
      console.log("🚀 ~ file: Members.svelte:127 ~ memberList:", memberList);
      ...
      <svg
        ...
        xmlns="http://www.w3.org/2000/svg"
      >
        <path fill-rule="evenodd" d="M2 10h16" stroke="currentColor" stroke-width="2" />
      </svg></Button
    >
  </a>
</li>
